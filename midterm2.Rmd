...
In the shiny ui part, first of all, I allow the input city selections to be the cities we obtained from task2 and a second quantity selection option. The second selection at ui step would only include a none variable called `no selection ` This quantity will be updated in there server step by retrieving all the numerical column names returned by the get_darksky function reactively. Then I create a checkbox to check whether the user want the historical data. If the box is checked, I will allow the user to select date to check for historical forecast data. Last but not least step I create in ui step is conditional check for display plots. If the second quantity is not selected, the plot will only be plotted of temperature against the time.  And if the second quantity is selected, the plot will be displayed with both temperature and selected quantity against the time. 
In the server part, I create four variable called `location`, `lat`, `long` and `date` reactively to the user input. `location` is generated by the selected city with geocode function, and `long` and `lat` are taking from this value. The Then I reactively generate selection options by taking to numerical columns’ names from `results`. Therefore, the selection options will be updated every time according to user’s input values. Meanwhile, once the city is selected, the latitude and longitude table will be produced displaying the location respect to that city. Finally the plot will be displayed based on user’s input conditions. If the user chooses a second quantity, the plot will be reflected by two facets plot of temperature and the second quantity respecting to time. Otherwise, the plot will only present temperature versus time. 
